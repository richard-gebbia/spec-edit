...
initWithFlags 
    : (flags -> (model                    , Cmd msg)) 
    -> flags -> (UndoList (model, Cmd msg), Cmd (UndoList.Msg msg))
...
update 
    ...
    ->  UndoList.Msg msg -> UndoList (model, Cmd msg) -> (UndoList (model, Cmd msg), Cmd (UndoList.Msg msg))
update updateFn = 
    let 
        innerUpdate msg undolist =
            case msg of
                ...
                UndoList.New innerMsg ->
                    let
                        (updated, cmd) = updateFn innerMsg (fst undolist.present)
                        nextUndoList = UndoList.new (updated, cmd) undolist
                    in
...
subLift subFn =
    .present >> fst >> subFn >> Sub.map UndoList.New
